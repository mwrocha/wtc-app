...
class ProfileViewModel(
    ...
    fun loadProfile(clientId: String) {
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.update {
                    it.copy(
                        client = client, groups = groups, divisions = divisions, isLoading = false
                    )
                }
                loadAvatar()
            } catch (e: Exception) {
    ...
    fun uploadAvatar(uri: Uri, context: Context) {
        viewModelScope.launch {
            ...
            try {
                ...
                val part =
                    MultipartBody.Part.createFormData("file", "avatar.$extension", requestBody)
                ...
                _uiState.update {
                    it.copy(
                        isUploadingAvatar = false, avatarError = "Erro: ${e.message}"
                    )
                }
            }
        }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val fieldsOk = try {
                    ...
                    android.util.Log.w(
                        "ProfileViewModel", "updateClientProfile fields: ${e.message}"
                    )
                ...
                if (nameOk || fieldsOk) {
                    ...
                    _uiState.update {
                        it.copy(
                            isLoading = false, error = "Falha ao atualizar perfil."
                        )
                    }
                }
            } catch (e: Exception) {
                android.util.Log.w(
                    "ProfileViewModel", "updateClientProfile exception: ${e.message}"
                )
                _uiState.update {
                    it.copy(
                        client = client.copy(
    ...
    fun changePhone(newPhone: String) {
        ...
        viewModelScope.launch {
            ...
            repository.changePhone(client.id, newPhone).onSuccess {
                _uiState.update {
                    it.copy(
                        isLoading = false, phoneSuccess = true, client = it.client?.copy(
                            phone = newPhone, tags = it.client.tags ?: emptyList()  // ← fix
                        )
                    )
                }
            }.onFailure { e ->
                _uiState.update {
                    it.copy(
                        isLoading = false, phoneError = e.message ?: "Erro ao alterar telefone."
                    )
                }
            }
        }
    ...
    fun changeCpf(newCpf: String) {
        ...
        viewModelScope.launch {
            ...
            repository.changeCpf(client.id, newCpf).onSuccess {
                _uiState.update {
                    it.copy(
                        isLoading = false, cpfSuccess = true, client = it.client?.copy(
                            cpf = newCpf, tags = it.client.tags ?: emptyList()  // ← fix
                        )
                    )
                }
            }.onFailure { e ->
                _uiState.update {
                    it.copy(
                        isLoading = false, cpfError = e.message ?: "Erro ao alterar CPF."
                    )
                }
            }
        }
    ...
    fun changeCompany(newCompany: String) {
        ...
        viewModelScope.launch {
            _uiState.update {
                it.copy(
                    isLoading = true, companyRequestError = null, companyRequestSuccess = false
                )
            }
            repository.changeCompany(client.id, newCompany).onSuccess {
                _uiState.update {
                    it.copy(
                        isLoading = false, companyRequestSuccess = true, client = it.client?.copy(
                            company = newCompany, tags = it.client.tags ?: emptyList()  // ← fix
                        )
                    )
                }
            }.onFailure { e ->
                _uiState.update {
                    it.copy(
                        isLoading = false,
                        companyRequestError = e.message ?: "Erro ao alterar empresa."
                    )
                }
            }
        }
    ...
    fun clearPhoneState() {
        _uiState.update { it.copy(phoneSuccess = false, phoneError = null) }
    }

    fun clearCpfState() {
        _uiState.update { it.copy(cpfSuccess = false, cpfError = null) }
    }

    fun clearCompanyRequestState() {
        _uiState.update { it.copy(companyRequestSuccess = false, companyRequestError = null) }
    }
